...
namespace kenai::spatial {
    ...
    class Texture3D : public kenai::core::Texture {
    private:
    ...
    public:
        ...
        std::vector<kenai::core::Image> getData() {
            std::vector<kenai::core::Image> images;
            TypedArray<GodotImage> arr = texture->get_data();
            for (int i = 0; i < arr.size(); i++) {
                Ref<GodotImage> img = arr[i];
                if (img.is_valid()) {
                    images.push_back(kenai::core::Image(img.ptr()));
                }
            }
            return images;
        }
